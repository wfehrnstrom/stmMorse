 ...
 * @retval None
 */
int32_t Accelero_Sensor_Handler( void *handle )
{
    ...
    if ( status == 1 )
    {
        ...
        {
            sprintf(dataOut, "COMPONENT ERROR!\n\r");
            CDC_Fill_Buffer(( uint8_t * )dataOut, strlen( dataOut ));
    ...
    //  sprintf(dataOut, "Acceleration Z: %d\n\r", (int) acceleration.AXIS_Z);
    //  CDC_Fill_Buffer((uint8_t *) dataOut, strlen( dataOut ));
    return acceleration.AXIS_Z;
}
 ...
 * @retval None
 */
uint32_t Gyro_Sensor_Handler( void *handle)
{
    uint8_t id;
    ...
    //state[0] = angular_velocity.AXIS_X;
    return angular_velocity.AXIS_X;
}
